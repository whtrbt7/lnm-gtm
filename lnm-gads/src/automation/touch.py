...
class TouchAutomation:
        ...
        for camp_row in campaign_rows:
            ...
            try:
                response = criterion_service.mutate_campaign_criteria(
                    customer_id=self.cid, operations=ops
                )
                total_added += len(response.results)
            except GoogleAdsException as e:
                error_code = e.error.code().name if e.error else "UNKNOWN"
                print(f"  Skip campaign {campaign_rn}: {error_code}")
            except Exception as e:
                print(f"  Skip campaign {campaign_rn}: {e}")
    ...
    def audit_asset_health(self) -> None:
        ...
        # TODO: Query ad_group_ad_asset_view, identify "LOW" performance
        pass
    ...
    def audit_wasted_spend(self) -> None:
        ...
        try:
            ...
            self.reporter.log_issue(
                ...
                f"{len(wasted)} search terms with ≥50 impressions and <0.5% CTR (irrelevant traffic)",
    ...
    def run_all(self) -> ReportingService:
        ...
        print(f"[{self.cid}] Light Touch complete.")
